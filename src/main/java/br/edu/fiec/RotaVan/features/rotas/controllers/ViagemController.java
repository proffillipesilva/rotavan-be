...
public class ViagemController {
    ...
    @PostMapping
    public ResponseEntity<?> createViagem(@Valid @RequestBody Viagem viagem) {
        try {
            Viagem savedViagem = viagemService.save(viagem);
            return new ResponseEntity<>(savedViagem, HttpStatus.CREATED);
        } catch (IllegalArgumentException e) { // Captura erro de hora
            return ResponseEntity.badRequest().body(e.getMessage());
        } catch (RuntimeException e) { // Captura erro de Motorista/Crianca/Rota não encontrado
